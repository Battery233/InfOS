...
{
  // file size ==0 or buffer size ==0, cannot read
  if (off >= this->size()||!this->size()||!size) return 0;
  char* string = (char *)buffer;
  int start_block_num = off/512;
  ...
  int j = 0;
  for(off_t i = off%512;i<off%512+size_to_read;i++){
    string[j++] = read_blocks_needed[i];
  }
  delete read_blocks_needed;
